...
$strquery = "SELECT * FROM compra,pedidos,insumos where id_in = id_in_pe and id_compra = id_compra_pe  and estado_pe = 2  ORDER BY id_compra";
...
if (!isset($_GET['fecha_incial']) and !isset($_GET['fecha_final'])) {
        ...
        $strquery6 = "SELECT * FROM compra,pedidos,insumos where id_in = id_in_pe and id_compra = id_compra_pe  and estado_pe = 2  ORDER BY id_in";
 ...
 $data = new Conexion1();
$conexion = $data->conect();
$strquery = "SELECT * FROM compra,pedidos,insumos where id_in = id_in_pe and id_compra = id_compra_pe  and estado_pe = 2  AND fecha_pedido BETWEEN '{$fecha_inicio}' AND '{$fecha_final}' ORDER BY id_compra";
...
$data = $result->fetchall(PDO::FETCH_ASSOC);
        ...
        $strquery6 = "SELECT * FROM compra,pedidos,insumos where id_in = id_in_pe and id_compra = id_compra_pe  and estado_pe = 2  AND fecha_pedido BETWEEN '{$fecha_inicio}' AND '{$fecha_final}' ORDER BY id_in";
...
$_SESSION[0]['id_compra']=$data[0]['id_compra'];
...
$this->setXY(15, 42);
$this->Cell(100, 8, utf8_decode('RIF:12312312312'), 0, 0, 'L', 0);
$this->setXY(120, 35);
...
if (!isset($_GET['fecha_incial']) and !isset($_GET['fecha_final'])){   
    $this->Cell(100, 8, ucwords(strtolower(utf8_decode('Reporte General de Pedidos '))), 0, 1, 'C', 0);
$this->Ln(5);
}else{
       $this->Cell(100, 8, ucwords(strtolower(utf8_decode("Reporte de Pedidos"))), 0, 1, 'C', 0);
             $this->SetX(50);
...
if ($i==0) {
       $_SESSION['id_compra']=$data[$i]['id_compra'];  
     $pdf->SetFont('Times', 'B', 20);
$pdf->SetDrawColor(61,61, 61);
$pdf->SetFont('Times','b',10);
$pdf->Cell(180, 8, ucwords(strtolower(utf8_decode('Orden de compra N°:'.$_SESSION['id_compra']))), 1, 1, 'C', 0);
...
if ($_SESSION['id_compra']!=$data[$i]['id_compra']) {
$b=0;
...
// esto no lo mencione en el video pero también pueden poner la alineación de cada COLUMNA!!!
$pdf->SetAligns(array('C','C','C','c'));
$fecha=date("d/m/Y", strtotime($data[$i]['fecha_pedido']));
    $pdf->Row(array($b + 1, $data[$i]['cod_in'], ucwords(strtolower(utf8_decode($data[$i]['nom_in']))), round($data[$i]['cantidad_pe'],2), ucwords(strtolower(utf8_decode($unidad))), $fecha), 10);
 $b++; 
...
*/
        $strquery3 = "SELECT * FROM compra,pedidos,insumos where id_in = id_in_pe and id_compra = id_compra_pe  and estado_pe = 2  and id_in = '{$id_in}'  ORDER BY id_in";
        if (isset($_GET['fecha_incial']) and isset($_GET['fecha_final'])) {

    $strquery3 ="SELECT * FROM compra,pedidos,insumos where id_in = id_in_pe and id_compra = id_compra_pe  and estado_pe = 2 and id_in = '{$id_in}' AND fecha_pedido BETWEEN '{$fecha_inicio}' AND '{$fecha_final}' ORDER BY id_in";
        }
...
$pdf->Row(array($b + 1, $data6[$i]['cod_in'], ucwords(strtolower(utf8_decode($data6[$i]['nom_in']))), round($cantidad,2), ucwords(strtolower(utf8_decode($unidad)))), 10);
$b++;          
